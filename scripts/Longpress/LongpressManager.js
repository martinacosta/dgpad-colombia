function LongpressManager(_canvas) {
    ...
    var createFrameConstruction = function() {
      ...
      canvas.addText("<p>Pasos de la Construcción<input type=\"button\" id=\"copyText\" style=\"display:block; margin: 0 auto;\" value=\"Copy to Clipboard\" /></p><div id=\"ConsText\">" + contenido + "</div>", x, y, 250, 200, "c:rgba(59,79,115,0.18);s:3;r:15;p:4");

      /*
      var frame = new GUIElement(_canvas, "div");
      var span = document.createElement("span");
      var texto = document.createTextNode("Hola mundo !!!");
      span.appendChild(texto);
      frame.getDocObject().appendChild(span);
      frame.setStyle("height", "50px");
      frame.setAbsolute();
      frame.setColor("#FF0000");
      frame.addDblClickEvent(function(e) {
        this.getElementsByTagName("span").style.backgroundColor = "grey";

      })
      frame.getDocObject().addEventListener("mouseout", function () {
        var elementos = Array.from(this.getElementsByTagName("span"));
        elementos.map(function(span) {
          span.style.backgroundColor = "yellow";
        });
      })
      canvas.getDocObject().parentNode.appendChild(frame.getDocObject());
//      closebox = new CloseBox(frame, deleteTeX);
*/
    };
