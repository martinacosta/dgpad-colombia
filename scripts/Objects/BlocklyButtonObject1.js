...
function BlocklyButtonObject(_construction, _name, _display_name, _x, _y) {
  ...
  this.paintObject = function(ctx) {
    ...
    ctx.fillText(LABEL, X - BTN.w - 20, BTN.y+BTN.h*this.getFontSize()*0.2/2);
    ctx.strokeStyle = this.getColor().getRGB();
    if (BTN.mouseInside) ctx.lineWidth = 3 * 1.5;
    else ctx.lineWidth = 3;
    ctx.fillStyle = fs;
    drawButton(ctx, BTN.x, BTN.y, BTN.w*this.getFontSize()*0.2, BTN.h*this.getFontSize()*0.2, 10);
