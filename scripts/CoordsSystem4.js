...
//MEAG >Modificación para que las figuras salgan proporcionales en todas las pantallas
    me.getSource = function() {
		...
		// unid=width/Unit;
        // var txt = "SetCoords(widthWindow()*" + centerx + ",heightWindow()*" + centery + ",widthWindow()/"+unid+"," + Cn.is3D() + "," + window.innerWidth + "," + window.innerHeight + ");\n";
		// txt+="var lastWindoww="+window.innerWidth+";\n var lastWindowh="+window.innerHeight+";\n "
		var txt="var escala=widthWindow()/window.innerWidth";
		txt+ = "SetCoords(" + x0 +"*escala," + y0 + "*escala," + Unit + "*escala," + Cn.is3D() + "," + window.innerWidth + "," + window.innerHeight + ");\n";
        return txt;
    };
//fin modificación
    // MEAG start
