...
function ParallelConstructor() {
    ...
    this.newObj = function(_zc, _C) {
        return new ParallelLineObject(_zc.getConstruction(), "_r", _C[0], _C[1]);
    };
    ...
    this.preview = function(ev, zc) {
		...
		if (len > 0 && selection[0].isInstanceType("circle")) {
			texto2= $L.tool_Parallel_help_2f;
		}
		if (len > 0 && selection[0].getCode()=="vector") {
			texto2= $L.tool_Parallel_help_2g;
		}
		if (len > 0 && selection[0].getCode()=="arc3pts") {
			texto2= $L.tool_Parallel_help_2h;
		}
		if (len == 0) {
			texto2=$L.tool_Parallel_help_2a;
		}
		if (len == 2) {
			texto2=$L.tool_Parallel_help_2_intersection;
		}
		//fin MEAG
		...
		ctx.fillText($L.tool_Parallel_help_1+this.getC(0).getName()+texto2,zc.mouseX(ev)+40, zc.mouseY(ev));
